...
export default function MyAccount() {
    ...
    const handlePasswordSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        await dispatch(passwordChange(newPassword))
        if(status=='succeeded'){
            dispatch(logout())
            navigate('/login')
        }
...
<div className="mx-4 min-h-screen max-w-screen-xl sm:mx-8 xl:mx-auto">
  <h1 className="border-b py-6 text-4xl font-semibold">My Account</h1>
  <div className="grid grid-cols-8 pt-3 sm:grid-cols-10">
    ...
    <div className="col-span-8 overflow-hidden rounded-xl sm:bg-gray-50 sm:px-8 sm:shadow">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between">
        <p className="text-gray-600">Your email address is <strong>{loginUser?.email || 'N/A'}</strong></p>
          ...
          <label htmlFor="login-password">
            ...
            <div className="relative flex overflow-hidden rounded-md border-2 transition focus-within:border-blue-600">
              <input value={newPassword}
                    onChange={(e) => setNewPassword(e.target.value)} type="password" id="login-password" className="w-full flex-shrink appearance-none border-gray-300 bg-white py-2 px-4 text-base text-gray-700 placeholder-gray-400 focus:outline-none" placeholder="***********" />
            </div>
          </label>
        </div>
      ...
      <button type='submit' className="mt-4 rounded-lg bg-emerald-600 px-4 py-2 text-white">Save Password</button>
      <hr className="mt-4 mb-8" />
      </form>
      <div className="mb-10">
